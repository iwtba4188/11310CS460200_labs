...
class Activation:
    ...
    def backward(self, dA=None, Y=None):
        if self.activation_function == "sigmoid":
            ...
            Z = np.where(
                self.cache >= 0,
                1 / (1 + np.exp(-self.cache)),
                np.exp(self.cache) / (1 + np.exp(self.cache)),
            )
            dZ = dA * Z * (1 - Z)
            ### END CODE HERE ###
            ...
            ### START CODE HERE ###
            b = np.max(self.cache, axis=1, keepdims=True)
            s = np.exp(self.cache - b) / np.sum(
                np.exp(self.cache - b), axis=1, keepdims=True
            )
            dZ = s - Y
            ### END CODE HERE ###
